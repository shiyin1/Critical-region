...
!  mpi2drho3=dVdrho4
!  write(*,*)Tn(1,1:20)
!  write(*,*)sqrt(mpi2(1:20))

  msg2=dVdrho1+2.Q+00*rho*dVdrho2
!  msg2drho1=3.Q+00*dVdrho2+2.Q+00*rho*dVdrho3
!  msg2drho2=5.Q+00*dVdrho3+2.Q+00*rho*dVdrho4
!  msg2drho3=7.Q+00*dVdrho3+2.Q+00*rho*dVdrho5
!  write(*,*)sqrt(msg2(1:20))
...
!  naqkdx3=dnadx(:,4)
  ...
  call cal_BB(k,mpi2,msg2,dnbpidx,dnbsgdx,BBall,gridnum)
  B2B2=BBall(:,1)
!  write(*,*)B2B2(1:gridnum)
  B3B2=BBall(:,2)
  B4B2=BBall(:,3)
  B5B2=BBall(:,4)
  B2B3=BBall(:,5)
  ...
  call cal_F(k,mqk2,dnfdx,dnadx,Fall)
  F2=Fall(:,1)
!  write(*,*)F2(1:gridnum)
  F3=Fall(:,2)
  F4=Fall(:,3)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  etaq=0.Q+00
  etaphi=((-4.Q+00*(-2.Q+00 + etaq)*F3 + (-3.Q+00 + 2.Q+00*etaq)*F2)*h**2.Q+00*Nc +              &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i=1,anum
    ...
    summationV0=0.Q+00
      do j=1,gridnum
        summationb0=summationb0+b(1)*rho(j)*etaphi(j)*Tn(i,j)
        summationV0=summationV0+dVdkdrho0(j)*Tn(i,j)
          do n=2,anum
            summationbn=summationbn+b(n)*rho(j)*etaphi(j)*Tn(n,j)*Tn(i,j)
          end do
      end do
      flowbn=2.Q+00/(gridnum*k)*summationbn
      flowb0=1.Q+00/(gridnum*k)*summationb0
      flowV0=2.Q+00/gridnum*summationV0
    dadk(i)=flowbn+flowb0+flowV0
  end do

!  dVdkdrho1=k34pi2*(3.Q+00*lb1pi+lb1sg-4.Q+00*Nf*Nc*lf1qk)
!  dVdkdrho2=k34pi2*(3.Q+00*lb2pi+lb2sg-4.Q+00*Nf*Nc*lf2qk)

  dydx=0.Q+00
  dydx(1:anum)=dadk(1:anum)
!  dydx(1:gridnum)=dVdkdrho1(1:gridnum)
!  dydx((1+gridnum):(2*gridnum))=dVdkdrho2(1:gridnum)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  write(*,*)'k=',k
!  if(k<200)then
!   open(unit=12,file='./buffer/a.dat',position='append')
!   call phyphi(a,V0,imin)
!   write(12,*)k
!   write(12,*)a(1:gridnum)
!   close(12)
!   end if
!  stop
end
