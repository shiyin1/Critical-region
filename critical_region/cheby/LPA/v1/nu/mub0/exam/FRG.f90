...
!Yong-Rui Chen 2020.1.6
  ...
  integer KMAXX,NMAX
  parameter (NMAX=800,KMAXX=800)
  real(16) dxsav,xp(KMAXX),yp(NMAX,KMAXX) 
  !variables in common block of subroutine odeint
  real(16) h,cbar,delta,t1,t2,it
  ...
  common /Tmu/ T,mu
  common /gridrho/ gridnum,rho,rhomin,rhomax,maxim
  ...
  delta=exp(log(t1)-(log(t1)-log(t2))/it*real((j1-1),kind=16))
  
  T=145.0118Q+00+delta !v3
  mu=0.Q+00
...
!  gridnum=131 !v2
  ...
  do i=1,gridnum
  rho(i)=rhomax/2.Q+00*cos(pi*(i-0.5Q+00)/gridnum)+rhomax/2.Q+00
  do j=1,gridnum
...
! grid on
  ...
  call initial(a)
  call odeint(a,gridnum,k_UV,k_IR,eps_ode,h1,hmin,nok,nbad,derivs,rkqs)
...
!  write(50,*) c
!  close(50)
  open(unit=50,file='./buffer/mu.dat',position='append')
    write(50,*)mu
  close(50)
