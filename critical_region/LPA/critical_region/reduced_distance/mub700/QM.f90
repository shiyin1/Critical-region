...
!order of chebyshev polynomial
  ...
  real(16) chebev,cl,Tc,rc1,r2,muc,r,a,b
  external chebev
  ...
  rc1=-11.Q+00
  r2=-3.Q+00
  a=2.9668660400640925
  b=-2011.9801200448637

  do i=1,iTmax
    r=exp((rc1)+((-rc1)-(-r2))/iTmax*real((i-1),kind=16))

    muB_MeV=(-(a*b*muc**2) + a*muc**2*Tc + muc*Tc**2)/(a**2*muc**2 + Tc**2) - Sqrt(-(b**2*muc**2*Tc**2) &
          - 2*a*b*muc**3*Tc**2 - a**2*muc**4*Tc**2 + a**2*muc**4*r**2*Tc**2 + 2*b*muc**2*Tc**3 +     &
          2*a*muc**3*Tc**3 - muc**2*Tc**4 + muc**2*r**2*Tc**4)/(a**2*muc**2 + Tc**2)
    muBi=muB_MeV/hc
    T_MeV=a*muB_MeV+b
    ...
    open(unit=51,file='./buffer/r.dat',position='append')
    write(51,*)r
    close(51)
