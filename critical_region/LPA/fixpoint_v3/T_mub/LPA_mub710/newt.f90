subroutine newt(x, n, check, functionn)
  ...
  parameter (NP=40, MAXITS=200, TOLF=1.Q-12, TOLMIN=1.Q-14, TOLX=1.Q-14, STPMX=1000.Q+0)
...
!lnsrch returns new x and f. It also calculates fvec at the new x when it calls fmin
	...
	test=0.Q+0                            !Test for convergence on delt-x
	do i=1, n
...
!  if(slope >= 0.0) pause 'roundoff problem in lnsrch'
  ...
  test=0.Q+0
  do i=1, n
    temp=abs(p(i))/max(abs(xold(i)),1.Q+0)
	if(temp.gt.test)test=temp
  end do
	...
	if(alam.lt.alamin) then
	  ...
	  if(alam.eq.1.Q+0) then
		...
		if(a.eq.0.) then
		  ...
		  if(disc.lt.0.Q+0) pause 'roundoff problem in lnsrch'
		  tmplam=(-b+sqrt(disc))/(3.Q+0*a)
		end if

		if(tmplam.gt.0.5Q+0*alam) tmplam=0.5Q+0*alam
...
subroutine ludcmp(a, n, np, indx, d)  !¾ØÕóµÄLU·Ö½â
  ...
  parameter (NMAX=10,TINY=1.0Q-21)
  integer i, imax, j, k
