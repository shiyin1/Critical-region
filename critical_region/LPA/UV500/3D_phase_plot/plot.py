...
l=10
ax1.plot(pb[:,0],pb[:,1]*0-l,pb[:,1],'-',c='b',linewidth=1.,alpha=1.,label=r'$\mu_B=0$')

ax1.fill_between(mub, mub*0-l, beta0p38 ,mub,mub*0-l, pb[:,1], color='skyblue', alpha=0.5, edgecolor='none',zorder=0)  
ax1.fill_between(mub, mub*0-l, beta0p37 ,mub,mub*0-l, pb[:,1], color='skyblue', alpha=0.5, edgecolor='none',zorder=0)  
ax1.fill_between(mub, mub*0-l, beta0p36 ,mub,mub*0-l, pb[:,1], color='skyblue', alpha=0.5, edgecolor='none',zorder=0)  
ax1.fill_between(mub,delta*0-l, pb[:,1] ,mub, np.log(delta), pb[:,1] , color='#F4DDB6', alpha=0.5, edgecolor='none',zorder=0)  
#ax1.fill_between(mub,delta*0-3*l, pb[:,1] ,mub, (delta), pb[:,1] , color='#F4DDB6', alpha=0.5, edgecolor='none',zorder=0)  
#ax1.fill_between(mub2,delta*0, pb2[:,1], mub2, delta*0, nu , color="#EFA7BF", alpha=0.5, edgecolor='none',zorder=0)  
ax1.quiver(0, 0-l, pb[0,1], 0, l, 0, arrow_length_ratio=0.1, color='b', linewidth=1.)
#黑色主坐标轴（真实物理轴）
ax1.quiver(0, -l, 0, 700, 0, 0, arrow_length_ratio=0.05, color='k', linewidth=1.5)
ax1.quiver(0, -l, 0, 0, 12, 0, arrow_length_ratio=0.1, color='k', linewidth=1.5)
ax1.quiver(0, -l, 0, 0, 0, 160, arrow_length_ratio=0.15, color='k', linewidth=1.5)
