...
mub600data=np.loadtxt('./mub530data.dat')
beta=0.759
# Create figure
...
ax1.plot(np.exp(mub450data[:,0]),mub450data[:,1],'-',c='#CEA2B5',linewidth=2.5,alpha=1.,label=r'$\mu_B=450\,\mathrm{MeV}$')
ax1.plot(np.exp(mub500data[:,0]),mub500data[:,1],'-',c='#9C9CC0',linewidth=2.5,alpha=1.,label=r'$\mu_B=500\,\mathrm{MeV}$')
ax1.plot(np.exp(mub600data[:,0]),mub600data[:,1],'-',c='#6F8BB8',linewidth=2.5,alpha=1.,label=r'$\mu_B=530\,\mathrm{MeV}$')
...
ax1.text(2*10**-5,0.735,r'$\mathrm{LPA}^\prime$',fontsize=15)
ax1.set_xscale('log')
ax1.axis([5*10**-6,0.015,0.65,0.8])

ax1.set_xlabel(r'$t$', fontsize=14, color='black')
ax1.set_ylabel(r'$\mathrm{Leading\,\,order\,\,Fit}\,\,\nu$', fontsize=14, color='black')

ax1.legend(loc=(0.15, 0.01),fontsize=9.5,frameon=False,shadow=True,handlelength=3.,borderpad=0.5,borderaxespad=1,scatterpoints=1)
